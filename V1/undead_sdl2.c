...
#define PRINCIPE "ppp.png"
/* **************************************************************** */

struct Env_t {
  SDL_Texture * background;
    SDL_Texture * principe;
  SDL_Texture * zombie;
  SDL_Texture * ghost;
  SDL_Texture * vampire;
  SDL_Texture * spirits;
...
Env * init(SDL_Window* win, SDL_Renderer* ren, int argc, char* argv[])
{
 ...
 /* init spirits texture from PNG image */
  ...
  SDL_FreeSurface(surf);
  TTF_CloseFont(font);
  /* init text texture using Arial font (PRINCIPE)*/
  font = TTF_OpenFont(FONT, 20);
  if(!font) ERROR("TTF_OpenFont: %s\n", FONT);
  TTF_SetFontStyle(font, TTF_STYLE_BOLD); // TTF_STYLE_ITALIC | TTF_STYLE_NORMAL
  surf = TTF_RenderText_Blended(font, "About ", color); // blended rendering for ultra nice text
  env->textprincipe = SDL_CreateTextureFromSurface(ren, surf);
  SDL_FreeSurface(surf);
  TTF_CloseFont(font);
...
void render(SDL_Window* win, SDL_Renderer* ren, Env * env)
{
 ...
 surf  = TTF_RenderText_Blended(font, b, color); // blended rendering for ultra nice text
  ...
  /* render About texture */
  SDL_QueryTexture(env->textprincipe, NULL, NULL, &rect.w, &rect.h);
  rect.x = 300; rect.y = 100;
  SDL_RenderCopy(ren, env->textprincipe, NULL, &rect);
...
bool process(SDL_Window* win, SDL_Renderer* ren, Env * env, SDL_Event * e)
{
  ...
  if (e->type == SDL_QUIT) {
    ...
    else if ((a>=292 && a<=392) && (b>=97 && b <= 123)){
      env->principe = IMG_LoadTexture(ren, PRINCIPE);
      if(!env->principe) ERROR("IMG_LoadTexture: %s\n", PRINCIPE);
      /* render background texture */
      SDL_RenderCopy(ren, env->principe, NULL, NULL); /* stretch it */
    }
    }
